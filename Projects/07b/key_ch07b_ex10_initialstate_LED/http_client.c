...
void http_client_task(void *arg){
	...
	while(1){
		// Wait until the button is pressed to send a message
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

		// Toggle the LED state
		LED_state = !LED_state;
		// Create the json representing the state of the LED
		if(LED_state){
			sprintf(eventValue, "{\"key\":\"Virtual LED\",\"value\":\"on\"}");
			printf("Sending LED On request.\n");
			cyhal_gpio_write(CYBSP_USER_LED, 0);
		}
		else{
			sprintf(eventValue, "{\"key\":\"Virtual LED\",\"value\":\"off\"}");
			printf("Sending LED Off request.\n");
		...
		if(connected){
			...
			if(result != CY_RSLT_SUCCESS){
				...
				// Request Sent
				if(LED_state){
					printf("LED On request sent!\n");
				}
				else{
					printf("LED Off request sent!\n");
				}
			}
		}
